...
public class Logic {
    ...
    void processMessage(Message message, BeautyBot bot) {
        ...
        switch (message.getText()) {
            case START -> newText = sendStartMessage(bot, buttons);
            case INSTRUCTION -> {
                sendInstructionMessage(bot);
                return;
            }
            case RETURN_TO_START -> {
                sendReturnToStartMessage(bot, buttons);
                return;
            }
        }
        if (bot.getUser().getState() == State.INGREDIENT) {
            newText = sendMessageStateIngredient(bot, message, buttons);
        }
        if (bot.getUser().getState() == State.ALL) {
            newText = sendMessageStateAll(bot, message, buttons);
        }
    ...
    private void sendReturnToStartMessage(BeautyBot bot, List<Button> buttons) {
        mapButtonsComponents.remove(bot.getUser().getChatId());
        bot.getUser().updateState(State.DEFAULT);
        updateState(bot.getUser(), bot);
        String newText = "1. Я смогу помочь тебе выбрать подходящее уходовое средство, проанализировав состав продукта. Также я расскажу всю необходимую информацию об активных компонентах, и ,кроме того, предупрежу об опасностях для твоей кожи, скрывающихся в составе.\n2. Еще я смогу ответить на твои вопросы об отдельных активных веществах: объясню для чего они применяются, как они влияют на твою кожу, в чем принцип их работы.";
    ...
    private String sendStartMessage(BeautyBot bot, List<Button> buttons) {
        String newText = "Привет," + bot.getUser().getUserName() + " меня зовут SkinHelper! Я помогу тебе разобраться в уходе за кожей лица.\nСо мной ты поймешь какие компоненты в косметике подойдут именно тебе!";
    ...
    private String sendMessageStateIngredient(BeautyBot bot, Message message, List<Button> buttons) {
        String newText;
        try {
    ...
    private String sendMessageStateAll(BeautyBot bot, Message message, List<Button> buttons) {
        String newText;
        try {
            ...
            for (String component : componentsList) {
                ...
                if (inf == null) {
                    ...
                    textGoodComponents.append('•').append(component).append(":\n").append(inf).append("\n\n");
            ...
            if (textGoodComponents.isEmpty()) {
                ...
                textGoodComponents.append(textComedogenic).append("Хотите ли узнать подробнее о каком-то из компонентов?");
                newText = textGoodComponents.toString();
    ...
    void processCallback(String callback, BeautyBot bot) throws SQLException {
        ...
        switch (callback) {
            case I_CAN -> newText = callbackICan(buttons);
            case STRUCTURE -> newText = callbackStructure(bot, buttons);
            case COMPONENT -> newText = callbackComponent(bot, buttons);
            case DETAILED -> newText = callbackDetailed(bot, buttons, component);
            case YES -> {
            ...
            case NO -> newText = callbackNo(bot, buttons);
        }
        Message answer;
        if (buttons.isEmpty()) {
    ...
    private String callbackICan(List<Button> buttons) {
        String newText = "1. Я смогу помочь тебе выбрать подходящее уходовое средство, проанализировав состав продукта. Также я расскажу всю необходимую информацию об активных компонентах, и ,кроме того, предупрежу об опасностях для твоей кожи, скрывающихся в составе.\n2. Еще я смогу ответить на твои вопросы об отдельных активных веществах: объясню для чего они применяются, как они влияют на твою кожу, в чем принцип их работы.";
        Button newButton1 = new Button("Анализ состава", STRUCTURE);
        Button newButton2 = new Button("Информация о компоненте", COMPONENT);
        buttons.add(newButton1);
    ...
    private String callbackStructure(BeautyBot bot, List<Button> buttons) {
        bot.getUser().updateState(State.ALL);
        updateState(bot.getUser(), bot);
        String newText = "Введите состав вашего продукта!";
    ...
    private String callbackComponent(BeautyBot bot, List<Button> buttons) {
        bot.getUser().updateState(State.INGREDIENT);
        updateState(bot.getUser(), bot);
        String newText = "Введите название актива";
    ...
    private String callbackDetailed(BeautyBot bot, List<Button> buttons, String component) throws SQLException {
        String newText = bot.getComponentBase().getDetailInfFromComponent(component, "components");
    ...
    private String callbackNo(BeautyBot bot, List<Button> buttons) {
        mapButtonsComponents.remove(bot.getUser().getChatId());
        String newText = "Чем займемся дальше?";
